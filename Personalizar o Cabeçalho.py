    ...
    def update_html(self, html_file):
        try:
            with open(html_file, 'r', encoding='utf-8') as f:
                html = f.read()
            import re
            html = re.sub(r'<h2>.*?</h2>', f'<h2>{self.nome_entry.get()}</h2>', html)
            html = re.sub(r'<p>Assistência.*?</p>', f'<p>{self.desc_entry.get()}</p>', html)
            html = re.sub(r'<p>Contato:.*?</p>', f'<p>Contato: {self.contato_entry.get()}</p>', html)
            if hasattr(self, 'logo_data'):
                html = re.sub(r'src="/images/logo.png"', f'src="{self.logo_data}"', html)
